"""
General object detection model using YOLOv8 (all COCO classes).
"""
...
class ObjectDetectionModel(BaseAIModel):
        ...
        try:
            if _CUSTOM_CONFIG.exists():
                try:
                    ...
                    self._logger.info(f"Object detection config loaded from {_CUSTOM_CONFIG}")
            ...
            self._logger.info(f"Object detection model loaded on {device}")
            ...
            self._logger.warning("ultralytics not installed — ObjectDetection disabled")
            return False
        except Exception as e:
            self._logger.error(f"Failed to load object model: {e}")
            return False
    ...
    def infer(self, frame: np.ndarray) -> List[Detection]:
        ...
        try:
            ...
            self._logger.error(f"Object detection error: {e}")
            return []
